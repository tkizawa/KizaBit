...
KizaBit は、WPF で動作するレトロ風の 8-bit コンピューターシミュレーターです。WIDTH コマンドで切り替え可能な CRT 風画面、簡易 Motorola 6809 CPU、Microsoft BASIC 風インタープリターを 1 つのウィンドウで操作できます。
...
- RUN: 登録済みプログラムを実行
- CLS: 画面をクリア
- WIDTH n[,m]: CRT領域全体を使ったまま文字数と行数を変更し、表示に合わせてフォントサイズを再計算してクリア
- CPU: CPU 状態の要約を表示
